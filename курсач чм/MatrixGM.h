		...
		for (int q = 0; q < stiffnessMatrix.size(); q++)
		{
			for (int p = q; p < stiffnessMatrix.size(); p++)
			{
				...
				for (int i = 0; i < _gaussSplitsNumber; i++)
				{
					...
					{
						double rI =
							((GIGAGRID->Nodes[GIGAGRID->Elems[elemIndex].NodeIndex[0]].r + r * _widthStep)
							+
							(GIGAGRID->Nodes[GIGAGRID->Elems[elemIndex].NodeIndex[0]].r + (r + 1) * _widthStep))
							/ 2.0 + _rootsLegendrePolynomial[i] * _widthStep / 2;
